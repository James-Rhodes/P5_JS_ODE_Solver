...
class doublePendulum extends RK4Solver{
    constructor(initVars,h){
        ...
        this.funcs = [this.theta1Func,this.theta1DotFunc,this.theta2Func,this.theta2DotFunc];
    ...
    theta1DotFunc(time,vars){
        ...
        let L1 = l1;
        let L2 = l2;
        let num = (-g*(2*M1+M2)*sin(theta1)) - (M2*g*sin(theta1-2*theta2)) - (2*sin(theta1-theta2)*M2*(w2*w2*L2 + w1*w1*L1*cos(theta1-theta2)));
    ...
    theta2DotFunc(time,vars){
        ...
        let num = (2*sin(theta1-theta2)*(w1*w1*L1*(M1+M2)+g*(M1+M2)*cos(theta1)+w2*w2*L2*m2*cos(theta1-theta2)));
